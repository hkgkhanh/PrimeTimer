...
document.onkeyup = function(event){
	...
	document.getElementById("stop_timer").value = "";
	...
	if (importFocus == 0 && keyPressed == 32 && inspectType_value == 1){
		...
		if (timerStatus % 3 == 2){
			...
			document.getElementById("ispt_display").innerHTML = "0";
			...
			document.getElementById("stop_timer").focus();
		};
		if (timerStatus % 3 == 0){
	...
	//last, next scramble
	if (keyPressed == 37 && event.altKey) {
		document.getElementById("last_scram_btn").click();
	};
	if (keyPressed == 39 && event.altKey) {
		generateScram();
	};
