//##### CALCULATE MEAN AND AVERAGE #####
			...
			function calc_ao5() {
				for (var ind10 = 0; ind10 < sessions[sesNum - 1].length - 4; ind10++) {
					var ind11 = 0;
					
					var array5 = [sessions[sesNum - 1][ind10], sessions[sesNum - 1][ind10 + 1], sessions[sesNum - 1][ind10 + 2], sessions[sesNum - 1][ind10 + 3], sessions[sesNum - 1][ind10 + 4]];

					var array5Time = [sessions[sesNum - 1][ind10].time, sessions[sesNum - 1][ind10 + 1].time, sessions[sesNum - 1][ind10 + 2].time, sessions[sesNum - 1][ind10 + 3].time, sessions[sesNum - 1][ind10 + 4].time];

					for (var ind12 = 0; ind12 < 5; ind12++) {
						if (array5[ind12].pen == "dnf") {
							ind11++;
						};
					};
					if (ind11 == 0) {
						...
						if (avg5 < bestAvg5) {
							...
							bestA5strngArray = [sessions[sesNum - 1][ind10].strng, sessions[sesNum - 1][ind10 + 1].strng, sessions[sesNum - 1][ind10 + 2].strng, sessions[sesNum - 1][ind10 + 3].strng, sessions[sesNum - 1][ind10 + 4].strng];
							bestA5TraoArray = [sessions[sesNum - 1][ind10].trao, sessions[sesNum - 1][ind10 + 1].trao, sessions[sesNum - 1][ind10 + 2].trao, sessions[sesNum - 1][ind10 + 3].trao, sessions[sesNum - 1][ind10 + 4].trao];
						};

					} else {
						if (ind11 == 1) {
							for (var ind13 = 0; ind13 < 5; ind13++) {
								if (array5[ind13] == "DNF") {
									array5Time[ind13] = 99999;
								};
							};
							array5Time.sort(compare);
